    ...
    public partial class FrmPermissionList : Form
    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (detail.PermissionID == 0)
                ...
                MessageBox.Show("You can not update any aprroved or disapproved Permission");
            else
            {
                FrmPermission frm = new FrmPermission();
                frm.isUpdate = true;
        ...
        private void FrmPermissionList_Load(object sender, EventArgs e)
        {
            ...
            if(!UserStatic.IsAdmin)
            {
                ...
                btnDelete.Hide();
                btnDisapprove.Hide();
                btnClose.Location = new Point(430, 17);
            }
        }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (rbStartDate.Checked)
                ...
                list = list.Where(x => x.FinishDate < Convert.ToDateTime(dpFinish.Value) && x.FinishDate > Convert.ToDateTime(dpStart.Value)).ToList();
        ...
        private void dataGridView1_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            detail.FinishDate = Convert.ToDateTime(dataGridView1.Rows[e.RowIndex].Cells[9].Value);
        ...
        {
            DialogResult result = MessageBox.Show("Are your Sure about deleting this permission", "Warning", MessageBoxButtons.YesNo);
            if(result == DialogResult.Yes) 
            {
                if (detail.State == PermissionStates.Approved || detail.State == PermissionStates.Disapproved)
                    MessageBox.Show("You cannot delete aprroved and disapproved permissions");
                else
                {
