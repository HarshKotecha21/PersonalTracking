    ...
    public partial class FrmSalaryList : Form
    {
        ...
        private void FrmSalaryList_Load(object sender, EventArgs e)
        {
            ...
            if(!UserStatic.IsAdmin)
            {
                btnUpdate.Hide();
                btnDelete.Hide();
                btnNew.Location = new Point(240, 12);
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (txtYear.Text.Trim() != "")
                list = list.Where(X => X.SalaryYear == Convert.ToInt32(txtSalary)).ToList();
            if (cmbMonth.SelectedIndex != -1)
                list = list.Where(x => x.MonthID == Convert.ToInt32(cmbMonth.SelectedValue)).ToList();
            if(txtSalary.Text.Trim() !="")
            {
                if (rbMore.Checked)
                    list = list.Where(x => x.SalaryAmount > Convert.ToInt32(txtSalary)).ToList();
        ...
        {
            DialogResult result = MessageBox.Show("Are you sure about deleting the selectrd Salary", "warning", MessageBoxButtons.YesNo);
            if (result == DialogResult.Yes)
            {
                SalaryBLL.DeleteSalary(detail.SalaryID);
                MessageBox.Show("Selected Salary was deleted");
                FillAllData();
                ClearFilters();
            }  
        }
